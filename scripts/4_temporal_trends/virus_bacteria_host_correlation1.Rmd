...
```{r}
### files
temporal_table=sprintf("%s/intermediate_files/prev_SGBs_cluster_assignments_long_abundance.tsv", find_rstudio_root_file())
...
```{r}
temporal_dt <- fread(sprintf("%s", temporal_table), header = T, sep = "\t")
...
```{r}
merge_dt$king_host_species <- str_c(merge_dt$kingdom, "@", merge_dt$host_genus, "@",merge_dt$species)
...
cor_long <- cor_long[, c("Bacteria_kingdom", "Bacteria_genus", "Bacteria_species") := tstrsplit(Bacteria, "@", fixed=TRUE)]

cor_long <- cor_long[, c("Virus_kingdom", "Virus_host_genus", "Virus_species") := tstrsplit(Virus, "@", fixed=TRUE)] %>%
...
ggsave(file = sprintf("%s/charts/correlation_virus_SGB_best_bacteria1.pdf", find_rstudio_root_file()), width = 8, height = 4)
