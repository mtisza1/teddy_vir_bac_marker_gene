...
big_corp <- cor_long %>%
  ...
  ggplot(
    aes(x = 1, y = avg_cor, fill = predicted)
  ) +
  ...
  theme_bw() +
  labs(x = "",
       y = "Average correlation between\nvirus SGB and bacterial genera") +
...
rank_dt <- cor_long %>%
  ...
  filter(any(predicted == "predicted_genus")) %>%
  ungroup() %>%
  group_by(Virus_species, predicted, Bacteria_genus, Virus_host_genus) %>%
  summarize(avg_cor = mean(correlation)) %>%
  ungroup() %>%
...
rank_dt %>%
  ggplot(aes(x = predicted, y = rank)) +
  geom_violin() + 
  geom_jitter(height = 0, alpha = 0.2, width = 0.1) +
  facet_wrap(~Virus_host_genus) +
  stat_compare_means(label = "p.signif") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15))) +
  scale_fill_manual(values = c("#E54E21", "#6C8645")) +
