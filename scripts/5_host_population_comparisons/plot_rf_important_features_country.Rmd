...
long_dt <- fread(sprintf("%s", long_table), sep = "\t", header = T) %>%
  ...
  mutate(genus = gsub("\\|s__.*", "", genus),
         kingdom = case_when(grepl("k__Bac", lineage) ~ "Bacteria", 
                           ...
                           TRUE ~ "other"),
         feature = case_when(kingdom == "Virus" ~ genus,
                             kingdom == "Bacteria" ~ species,
                             TRUE ~ "other")) %>%
  subset(select = c("rel_abundance", "sampleID", "species", "genus", "kingdom", "feature"))
...
SWE_ab_dt <- merge(long_dt, SWE_imp_dt, by.x = "feature", by.y = "V1") 
...
SWE_ab_meta_dt <- merge(SWE_ab_dt, meta_dt, by.x = "sampleID", by.y = "sample") %>%
  ...
  select(c(sampleID, feature, rel_abundance, rounded_DOL, Country)) %>%
  group_by(feature, rounded_DOL, Country) %>%
  ## relative abundance to itself over all samples from each time bracket
  summarize(avg_rel_abund = mean(rel_abundance)) %>%
  ungroup() %>%
  ## adding 0s for rounded_DOL where feature was never detected
  complete(feature, rounded_DOL, Country) %>%
  replace(is.na(.), 0)
...
SWE_ab_meta_dt %>%
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), strip.text.x = element_text(size=4))
...
FIN_ab_dt <- merge(long_dt, FIN_imp_dt, by.x = "feature", by.y = "V1") 
...
GER_ab_dt <- merge(long_dt, GER_imp_dt, by.x = "feature", by.y = "V1") 
...
```{r}
USA_imp_dt <- fread(sprintf("%s", ger_important), sep = "\t", header = T) %>%
  head(n = 12) %>%
  arrange(desc(avg_importance)) %>%
  select(-avg_importance)

USA_ab_dt <- merge(long_dt, USA_imp_dt, by.x = "feature", by.y = "V1") 
